...
>>>>>>> 57bc19ea1320d933e633ccd5b9cf1540f1389ba4
...
stuff. Before you use a package for the first time you need to install it on 
your machine, and then you should import it in every subsequent R session when you need it. You should already have installed the **`tidyverse`** package. This is an
...
R is amazing because it is open source, free and anyone can build and share software for it. Unfortunately, this also means it has evolved "organically" and different designers have applied different approaches to how their packages work and how data is seen and treated by them. R runs a set of packages that are called base packaages and these are not always consistant or intuitive, which makes learning R difficult.

The **`tidyverse`** package tries to address common issues that arise when
doing data analysis with some of the functions that come with R by creating a harmonious set of packages that share an underlying characteristics.

1. Design philosophy
2. Grammar
3. Data structures

We will use the **`tidyverse`** package to read, inspect and manipulate data.
...
tasks. It is build with robust programming language (C++) and designed to work efficiently with remote and large datasets.

The package **`tidyr`** addresses the common problem of wanting to reshape your data for plotting and use by different R functions. Sometimes we want data sets where we have one row per measurement. Sometimes we want a data frame where each measurement type has its own column, and rows are instead more aggregated groups - time series or strains. Moving back and forth between these formats is non-trivial, and **`tidyr`** gives you tools for this and more sophisticated data manipulation.
...
We'll read in our data using the `read_csv()` function, from the tidyverse package **`readr`** and we'll create an object called "surveys" which will be a data table or "tibble".
...
This is referred to as a "tibble". If you have used R before, you may have used data frames. Tibble data structure is very similar to a data frame. For our purposes the only differences are that:
...
1. In addition to displaying the data type of each column under its name, it
   ...
   screen.
2. Columns of class `character` are never converted into factors. This created a lot of headaches in the past.
